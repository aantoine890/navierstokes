...
int main(int argc, char **args) {
    ...
    PetscReal dt = 0.0001;                  // Pas de temps initial
    PetscReal T = 0.001;                   // Temps final
    PetscInt  N_steps = (PetscInt)(T/dt);// Nombre de pas de temps
    PetscReal Re = 10;                // Nombre de Reynolds
    PetscReal delta = 0.05;               // Paramètre de stabilisation
    ...
    for (int step = 0; step < N_steps; ++step) {
        ...
        for (int iter = 0; iter < max_newton && !converged; ++iter) {
            ...
            KSP ksp_newton_step;
            KSPCreate(PETSC_COMM_WORLD, &ksp_newton_step);
            KSPSetOperators(ksp_newton_step, J, J);
            KSPSetType(ksp_newton_step, KSPGMRES);
            KSPGMRESSetRestart(ksp_newton_step, 30);
            PC pc_step;
            KSPGetPC(ksp_newton_step, &pc_step);
            PCSetType(pc_step, PCILU);
            PCFactorSetLevels(pc_step, 1);
            KSPSetTolerances(ksp_newton_step, 1e-8, 1e-10, PETSC_DEFAULT, 2000);
            KSPSetFromOptions(ksp_newton_step);
            KSPSetUp(ksp_newton_step);

            if (avx2) {
                Mat MatFactor;
                KSPGetPC(ksp_newton_step, &pc_step);
                PCFactorGetMatrix(pc_step, &MatFactor);
            ...
            VecScale(F, -1.0);
            KSPSolve(ksp_newton_step, F, delta_u);
