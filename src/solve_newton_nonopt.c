            ...
            for (int j = 0; j < 4; ++j) {
                ...
                for (int alpha = 0; alpha < 3; ++alpha) {
                    for (int beta = 0; beta < 3; ++beta) {
                        ...
                        block4x4[alpha*4 + beta] = (em->Mloc[ii][jj]/dt) + em->A0loc[ii][jj] + 
...
int main(int argc, char **args) {
    ...
    // === Conditions de Poiseuille (tag 2) ===
    double y0 = 0.0, z0 = 0.0, a = 1.0, b = 1.0, Umax = 1.0;
    int count = 0;
    ...
    for (int step = 0; step < N_steps; ++step) {
        ...
        // Sauvegarde par pas de temps
        char filename[256];
        snprintf(filename, sizeof(filename), "results_newton/solution_step_%04d.data", step);
        PetscViewer viewer;
        PetscViewerASCIIOpen(PETSC_COMM_WORLD, filename, &viewer);
        PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_MATRIXMARKET);
        VecView(u_n, viewer);
