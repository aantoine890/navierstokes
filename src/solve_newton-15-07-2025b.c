...
int main(int argc, char **args) {
    ...
    PetscReal Re = 300;                // Nombre de Reynolds
    ...
    if (avx2) {
        ...
        PetscPrintf(PETSC_COMM_WORLD, "Avant KSPSOLVE 1 :%p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 
        KSPSolve(ksp_newton, test_b, test_x);
        PetscPrintf(PETSC_COMM_WORLD, "Avant KSPSOLve 2: %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 
        KSPSolve(ksp_newton, test_b, test_x);
        PetscPrintf(PETSC_COMM_WORLD, "ApresKSPSOLve 2: %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve);
    ...
    for (int step = 0; step < N_steps; ++step) {
        ...
        for (int iter = 0; iter < max_newton && !converged; ++iter) {
            ...
            PetscPrintf(PETSC_COMM_WORLD, "Before non linear jacobian : %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 
            ...
            PetscPrintf(PETSC_COMM_WORLD, "After non linear jacobian :  %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 

            VecScale(F, -1.0);

            PetscPrintf(PETSC_COMM_WORLD, "After vec scale:  %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 

            KSPSolve(ksp_newton, F, delta_u);

            PetscPrintf(PETSC_COMM_WORLD, "After ksp solve:  %p\n", ((PC_Factor *)(pc->data))->fact->ops->solve); 
